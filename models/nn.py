...
import clip
import math
from models.MotionMTA import MotionMTA
#import open_clip
...
class MotionTransformer(nn.Module):
    def __init__(self,
        ...
        '''
        model, _, _ = open_clip.create_model_and_transforms("ViT-H-14", device=torch.device('cuda'), pretrained="laion2b_s32b_b79k")
        self.model = model
        '''
        #Text Transformer
        ...
        for i in range(num_layers):
            self.temporal_decoder_blocks.append(
                MotionMTA(
                    ...
                    text_dim=text_latent_dim,
                    time_emb_dim=latent_dim * 4,
                    tmp_dim = 196,
        ...
        self.ln = nn.LayerNorm(self.input_feats)

    def encode_text(self, text, mask, device):
        with torch.no_grad():
        ...
        #if mask:
        #xf_out = self.text_mask(xf_out)
        return xf_proj, xf_out
    
    
    def text_mask(self, text):
        mask = torch.bernoulli(torch.ones(text.shape, device=text.device) * 0.01)
        return text * (1 - mask)
    ...
    def forward(self, x, timesteps, mask=None, length=None, text=None, xf_proj=None, xf_out=None):
        """
        x: B, T, D
        """
        ...
        if xf_proj is None or xf_out is None:
            xf_proj, xf_out = self.encode_text(text, mask, x.device) # xf_out (128,196,512)
        
        '''
        tokens = open_clip.tokenize(text)
        z = self.encode_with_transformer(tokens.to("cuda"))
        '''

        #time_emb = timestep_embedding(timesteps, self.latent_dim).unsqueeze(1) # (64, 1, 512)
        #time_emb = self.time_conv(self.time_embed(time_emb)) #time_emb (128,196,512)
        emb = self.time_embed(timestep_embedding(timesteps, self.latent_dim)) + xf_proj
        # x + time_emb
        #h = self.joint_embed(x) + time_emb
        ...
        for module in self.temporal_decoder_blocks:
            ...
            '''
            '''
            h, skip_connection = module(h, tt)
            skip = skip_connection if skip is None else skip_connection + skip # (128, 196, 512)
            '''
            h = module(h, xf_out, emb) #3.14:tt->xf_out             
            

        output = self.out(h).contiguous().view(B, T, -1)
        #output = self.conv2(self.act(self.conv1(skip.permute(0, 2, 1))))
        return output
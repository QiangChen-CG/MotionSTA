...
class MotionTransformer(nn.Module):
    ...
    def encode_text(self, text, mask, device):
        ...
        a = xf_out[text.argmax(dim = -1)]
        b = torch.arange(xf_out.shape[1])
    ...
    def forward(self, x, timesteps, mask=None, length=None, text=None,  xf_proj=None, xf_out=None):
        ...
        if xf_proj is None or xf_out is None:
            xf_proj, xf_out = self.encode_text(text, mask, x.device)
        ...
        skip = skip / len(self.temporal_decoder_blocks)

        #output = self.out(h).view(B, T, -1).contiguous()
        output = self.out(skip).view(B, T, -1).contiguous()
        return output
